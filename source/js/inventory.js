...
export class Inventory {
	...
	checkCraftingTableForItem() {
		...
		var slot1type = document
			.querySelector('.the-fucking-crafting-table > .slot-1')
			.getAttribute('data-blocktype');
		var slot2type = document
			.querySelector('.the-fucking-crafting-table > .slot-2')
			.getAttribute('data-blocktype');
		var slot3type = document
			.querySelector('.the-fucking-crafting-table > .slot-3')
			.getAttribute('data-blocktype');
		...
		switch (recipe) {
			...
			default: {
				...
				defaultSlot.className = defaultSlot.className
					.replace(
						new RegExp(
							'\\b' + globals.allblockclasses().trim().split(' ').join('\\b|\\b') + '\\b',
							'g'
