	...
	playerPrimaryAction(blockid) {
		...
		if (blItems.itemIsInstrument(selecteditem)) {
			...
			document
				...
				.classList.add('player-direction-' + direction + '-' + selecteditem + '-swing');
		...
		// TODO: replace with CSS transition
		{
			...
			if (
				selecteditem == 'axe' &&
				(blItems.itemIsCollectable(blocktype) || blItems.itemIsCutable(blocktype))
			) {
