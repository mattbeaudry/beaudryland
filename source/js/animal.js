		...
		document
			...
			.insertAdjacentHTML(
				...
				'<div data-id="' +
					...
					'" class="objectId-' +
					id +
					' the-fucking-deer deer-direction-down"></div>'
			);
	...
	initAnimalBrain(id) {
		...
		function anAnimalThought() {
			...
			if (document.querySelectorAll('.objectId-' + id).length != 0) {
				...
				//var enemydirection;
				var animalX = blUtil.getObjectCurrentCol(id);
				var animalY = blUtil.getObjectCurrentRow(id);
				...
				switch (randomDirection) {
					case 1:
						blMovement.moveObject('up', id, 'deer');
						break;
					case 2:
						blMovement.moveObject('down', id, 'deer');
						break;
					case 3:
						blMovement.moveObject('left', id, 'deer');
						break;
					case 4:
						blMovement.moveObject('right', id, 'deer');
						break;
				}
				//}
