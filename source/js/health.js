...
export class Health {
	...
	showObjectHealth(objectId, number) {
		...
		var healthlabelhtml =
			'<div class="damage-label damage-label-id-' + rid + '">' + number + '</div>';
		document
			.querySelector('.objectId-' + objectId)
			.insertAdjacentHTML('beforeend', healthlabelhtml);
	...
	reduceObjectHealth(objectId, amount) {
		var blockhealth = document
			.querySelector('.objectId-' + objectId)
			.getAttribute('data-blockhealth');
	...
	refillHearts() {
		[...document.querySelectorAll('.the-fucking-hearts ul .empty')].forEach(
			(el) => (el.className = '')
		);
	...
	removeHeart() {
		...
		var notEmpty = [...document.querySelectorAll('.the-fucking-hearts li')].filter(
