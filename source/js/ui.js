...
export class UI {
  ...
  setupUI() {
    ...
    [...document.querySelectorAll(".bui-toast-trigger")].forEach(function(el) {
      el.addEventListener("click", function () {
        var toastText = el.getAttribute("data-text");
    ...
    var triggerToast = function (text) {
      var id = 1;
      var toastHtml =
        '<div id="toast-' + id + '" class="bui-toast bui-toast-white bui-toast-bottomright">';
      toastHtml += '<div class="bui-toast-close">X</div>';
      toastHtml += "<p>" + text + "</p>";
      toastHtml += "</div>";
      document.body.insertAdjacentHTML('beforeend', toastHtml);
      [...document.querySelectorAll(".bui-toast-close")].forEach(function(closeEl) {
        closeEl.addEventListener("click", function () {
          closeEl.closest(".bui-toast").remove();
        });
      });
      setTimeout(function () {
        // console.log("timeout trig");
        var toastEl = document.querySelector("#toast-" + id);
    ...
    [...document.querySelectorAll(".bui-colorpicker .bui-colorpicker-input")].forEach(function(el) {
      el.addEventListener("click", function () {
        var palette = el.parentElement.querySelector(".bui-colorpalette");
        if (palette) {
    ...
    [...document.querySelectorAll(".bui-colorpicker .bui-colorpalette .bui-swatch")].forEach(function(el) {
      el.addEventListener("click", function () {
        var colorCode = getComputedStyle(el).backgroundColor;
        var colorpicker = el.closest(".bui-colorpicker");
        var colorInput = colorpicker.querySelector(".bui-colorpicker-input");
        colorInput.style.backgroundColor = colorCode;
        colorInput.value = colorCode;
        el.closest(".bui-colorpalette").style.display = 'none';
      });
    });

    [...document.querySelectorAll(".bui-colorpicker .bui-colorpicker-eraser")].forEach(function(el) {
      el.addEventListener("click", function () {
    ...
    [...document.querySelectorAll(".bui-menu")].forEach(function(el) {
      el.addEventListener("click", function () {
        var dropdown = el.querySelector(".bui-menu-dropdown");
        var isVisible = dropdown && getComputedStyle(dropdown).display !== 'none';
        [...document.querySelectorAll(".bui-menu-dropdown")].forEach(d => d.style.display = 'none');
        if (isVisible) {
      ...
      for (var i = 0; i < elm.length; i++) {
        ...
        document.addEventListener("mousemove", function (e) {
          if (focused) {
            ...
            var newVal =
              (newRot - min) / focused.getAttribute("data-rosetta") +
              parseInt(focused.getAttribute("min"));
            ...
            if (lastVal !== stepped) {
              //only trigger event if value is new
